...
def _variable(name, shape, initializer, use_fp16=False, trainable=True):
...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
      ...
      outputs = tf.nn.conv2d(inputs, kernel,
                             ...
                             padding=padding)
      print('here',outputs.get_shape())
      # exit()
      biases = _variable('biases', [num_output_channels],
...
def batch_norm_dist_template(inputs, is_training, scope, moments_dims, bn_decay):
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    num_channels = inputs.get_shape()[-1].value
    beta = _variable('beta', [num_channels], initializer=tf.zeros_initializer())
    gamma = _variable('gamma', [num_channels], initializer=tf.ones_initializer())

    pop_mean = _variable('pop_mean', [num_channels], initializer=tf.zeros_initializer(), trainable=False)
    pop_var = _variable('pop_var', [num_channels], initializer=tf.ones_initializer(), trainable=False)
